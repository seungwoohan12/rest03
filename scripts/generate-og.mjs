/**
 * OG Image Generator — HANU Creative Studio
...
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  <defs>
    ...
    <linearGradient id="hBoxGrad" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%"   stop-color="${C.royalBlue}"/>
  ...
  <!-- H 아이콘 박스 -->
  <rect x="92" y="108" width="116" height="116" rx="20" fill="url(#hBoxGrad)"/>
  <text
    x="150" y="188"
    text-anchor="middle"
  ...
  <text
    ...
    fill="${C.iceWhite}" letter-spacing="3">HANU</text>
  ...
  <text
    ...
    fill="${C.iceWhite}" opacity="0.55" letter-spacing="2">CREATIVE STUDIO</text>

  <!-- 구분선 -->
  <rect x="92" y="255" width="700" height="2" rx="1" fill="${C.royalBlue}" opacity="0.55"/>
  ...
  <text
    ...
    fill="${C.iceWhite}" opacity="0.92">Creative Video Studio</text>
  ...
  <text
    ...
    font-family="Arial, Helvetica, sans-serif"
    font-size="19"
    fill="#93C5FD" opacity="0.82">Interview · Brand Video · Short-form · AI Contents</text>
...
async function main() {
  ...
  await sharp(Buffer.from(svg)).png({ quality: 95, compressionLevel: 6 }).toFile(outFile)
